...
public:
  Image(
    ...
    size_t imageSize, int status, std::unique_ptr<uint8_t[]> &&data, size_t w, size_t h, size_t stride,
    size_t bitsPerPixel, size_t numChan, uint64_t frameId, pixel_format::PixelFormat pixFmt,
    int frameCount, int frameDrop);
  ...
  // frame stats over lifetime of camera
  int frameCount_;
  int frameDrop_;
private:
};
