  ...
  const std::string & nodeName, const std::string & val, std::string * retVal)
{
  *retVal = "UNKNOWN";
  ...
  if (!common_checks(np, nodeName, &msg)) {
    // See if it's a TLS node
    np = genicam_utils::find_TLS_node(nodeName, camera_, debug_);
    if (!common_checks(np, nodeName, &msg)) {
      return (msg);
    }
  }
  ...
  const std::string & nodeName, T2 val, T2 * retVal, const Spinnaker::CameraPtr & cam, bool debug)
{
  *retVal = set_invalid<T2>();
  ...
  if (!common_checks(np, nodeName, &msg)) {
    // See if it's a TLS node
    np = genicam_utils::find_TLS_node(nodeName, cam, debug);
    if (!common_checks(np, nodeName, &msg)) {
      return (msg);
    }
  }
...
void SpinnakerWrapperImpl::OnImageEvent(Spinnaker::ImagePtr imgPtr)
{
  ...
  std::unique_ptr<uint8_t[]> image_data(new uint8_t[imgPtr->GetHeight() * imgPtr->GetStride()]);
  ...
  if (imgPtr->IsIncomplete()) {
    ...
    // publish a blank image
    memset(image_data.get(), 0, imgPtr->GetHeight() * imgPtr->GetStride());

    ImagePtr img(new Image(
      t, 0, 0, 0, 0, 0, imgPtr->GetImageSize(),
      imgPtr->GetImageStatus(), std::move(image_data), imgPtr->GetWidth(), imgPtr->GetHeight(),
      imgPtr->GetStride(), imgPtr->GetBitsPerPixel(), imgPtr->GetNumChannels(),
      imgPtr->GetFrameID(), pixelFormat_, frameCount_, frameDrop_));
    ...
    converted_ = processor_.Convert(imgPtr, imgPtr->GetPixelFormat());
#if 0
    ...
    const int16_t brightness =
      computeBrightness_
        ? compute_brightness(
            pixelFormat_, static_cast<const uint8_t *>(converted_->GetData()), converted_->GetWidth(),
            converted_->GetHeight(), converted_->GetStride(), brightnessSkipPixels_)
        : -1;

    // To be safe create a copy of the image buffer. Just in case converted_ deallocs the buffer before we consume it.
    memcpy(image_data.get(), converted_->GetData(), converted_->GetHeight() * converted_->GetStride());

    ImagePtr img(new Image(
      t, brightness, expTime, maxExpTime, gain, stamp, converted_->GetImageSize(),
      converted_->GetImageStatus(), std::move(image_data), converted_->GetWidth(), converted_->GetHeight(),
      converted_->GetStride(), converted_->GetBitsPerPixel(), converted_->GetNumChannels(),
      converted_->GetFrameID(), pixelFormat_, frameCount_, frameDrop_));
