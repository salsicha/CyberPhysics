...
{
  const auto t =
    adjustTimeStamp_ ? rclcpp::Time(im->imageTime_) : rclcpp::Time(im->time_);
  ...
  if (pub_.getNumSubscribers() > 0) {
    ...
    bool ret =
      sensor_msgs::fillImage(*img, encoding, im->height_, im->width_, im->stride_, im->data_.get());
    if (!ret) {
  ...
  if (cameraInfoExtPub_->get_subscription_count() > 0) {
    cameraInfoExtMsg_.header.stamp = t;
    cameraInfoExtMsg_.actual_frame_rate = wrapper_->camera()->AcquisitionResultingFrameRate.GetValue();
    cameraInfoExtMsg_.uptime = wrapper_->camera()->DeviceUptime.GetValue();
    cameraInfoExtMsg_.voltage = wrapper_->camera()->PowerSupplyVoltage.GetValue();
    cameraInfoExtMsg_.current = wrapper_->camera()->PowerSupplyCurrent.GetValue();
    cameraInfoExtMsg_.frames_dropped = static_cast<int>(im->frameDrop_);
    cameraInfoExtPub_->publish(cameraInfoExtMsg_);
  }
  ...
  if (imageRawMetaPub_->get_subscription_count() != 0) {
    imageRawMetaMsg_.header.stamp = t;
    imageRawMetaMsg_.meta.brightness = im->brightness_;
    imageRawMetaMsg_.meta.exposure_time = im->exposureTime_;
    imageRawMetaMsg_.meta.max_exposure_time = im->maxExposureTime_;
    imageRawMetaMsg_.meta.gain = im->gain_;
    imageRawMetaMsg_.meta.camera_time = im->imageTime_;

    sensor_msgs::msg::Image::UniquePtr img(new sensor_msgs::msg::Image(imageMsg_));
    bool ret = sensor_msgs::fillImage(*img, encoding, im->height_, im->width_, im->stride_, im->data_.get());

    if (!ret) {
      RCLCPP_ERROR_STREAM(get_logger(), "fill image failed!");
    } else {
      imageRawMetaMsg_.image = std::move(*img);
      imageRawMetaPub_->publish(imageRawMetaMsg_);
    }
  }
}
...
void CameraDriver::startCamera()
{
  if (!cameraRunning_) {
    ...
    if (!cameraRunning_) {
      ...
      // exit the app so Docker will restart the container
...
bool CameraDriver::start()
{
  ...
  imageRawMetaPub_ = create_publisher<flir_camera_msgs::msg::ImageRawMetaData>("~/image_raw_meta", 1);
  cameraInfoExtPub_ = create_publisher<flir_camera_msgs::msg::CameraInfoExt>("~/camera_info_ext", 1);
  ...
  if (wrapper_->initCamera(serial_)) {
    ...
    cameraInfoExtMsg_.camera_info = cameraInfoMsg_;
    cameraInfoExtMsg_.serial = wrapper_->camera()->DeviceSerialNumber.ToString();
    cameraInfoExtMsg_.model = wrapper_->camera()->DeviceModelName.ToString();
    cameraInfoExtMsg_.firmware = wrapper_->camera()->DeviceFirmwareVersion.ToString();
  } else {
