...
static CNodePtr find_TLS_node(const std::string & path, CNodePtr & node, bool debug)
{
  // split off first part
  auto pos = path.find("/");
  const std::string token = path.substr(0, pos);  // first part of it
    ...
    if (std::string(childNode->GetName().c_str()) == token) {
      if (is_readable(childNode)) {
        if (pos == std::string::npos) {  // no slash in name, found leaf node
          ...
          const std::string rest = path.substr(pos + 1);
          return (find_TLS_node(rest, childNode, debug));
        }
      }
    }
  }
...
CNodePtr find_TLS_node(const std::string & path, Spinnaker::CameraPtr cam, bool debug)
{
  INodeMap & appLayerNodeMap = cam->GetTLStreamNodeMap();
  CNodePtr rootNode = appLayerNodeMap.GetNode("Root");
  CNodePtr retNode = find_TLS_node(path, rootNode, debug);
  return (retNode);
}
}  // namespace genicam_utils
}  // namespace spinnaker_camera_driver
