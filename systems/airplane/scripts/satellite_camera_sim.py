...
# Must match METERS_PER_DEG_LAT in demnav's heightmap.py and wildnav's
# satellite_cache.py: the rendered scene and the reference DEM/tiles have to
# share the same local-tangent georeferencing.
...
def _satellite_rgb(east: np.ndarray, north: np.ndarray) -> np.ndarray:
    # Kept in sync with _synthetic_satellite_rgb in wildnav's
    # satellite_cache.py — WildNav matches camera frames against tiles
    # rendered by that copy, so the two must stay numerically identical.
...
def _terrain_height(east, north):
    # Kept in sync with _synthetic_terrain_height in demnav's heightmap.py —
    # DemNav correlates the depth rendered here against the DEM generated by
    # that copy, so the two must stay numerically identical.
...
class SatelliteCameraSim(Node):
    ...
    def _gps(self, msg: NavSatFix) -> None:
        metres_per_deg_lon = METERS_PER_DEG_LAT * math.cos(
            math.radians(self.origin_lat)
        )
        self.north_m = (float(msg.latitude) - self.origin_lat) * METERS_PER_DEG_LAT
    ...
    def _camera_altitude(self) -> float:
        ...
        terrain = float(_terrain_height(self.east_m, self.north_m))
    ...
    def _tick(self) -> None:
        ...
        nadir_terrain = float(_terrain_height(self.east_m, self.north_m))
    ...
    def _make_image(self, width: int, height: int,
        ...
        msg.data = _satellite_rgb(east, north).tobytes()
        return msg
    ...
    def _make_depth(self, altitude: float, view_height: float, stamp) -> Image:
        ...
        depth = np.maximum(1.0, altitude - _terrain_height(east, north))
